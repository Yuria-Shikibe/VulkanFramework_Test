...
namespace Assets::Ctrl{
	...
	CC::Operation cameraMoveLeft{"cmove-left", CC::KeyBind(CC::Key::A, CC::Act::Continuous, +[]{
		if(!disableMove)Core::Focus::camera.move({-cameraMoveSpeed * Core::timer.globalDeltaTick(), 0});
	})};
	...
	CC::Operation cameraMoveUp{"cmove-up", CC::KeyBind(CC::Key::W, CC::Act::Continuous, +[]{
		if(!disableMove)Core::Focus::camera.move({0,  cameraMoveSpeed * Core::timer.globalDeltaTick()});
	})};
	...
	export{
        ...
        void load() {
            ...
            Core::input->scrollListeners.emplace_back([](const float x, const float y) -> void {
                if(Core::Focus::camera)
                    Core::Focus::camera->setTargetScale(

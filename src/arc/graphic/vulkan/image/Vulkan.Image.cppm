...
export namespace Core::Vulkan{
	namespace Util{
		...
		template <std::size_t N>
		VkDependencyInfo dependencyOf(
			const std::array<VkBufferMemoryBarrier2, N>& barriers
		){
			return{
				.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO,
	...
	public:
		...
		void exportToBuffer(VkCommandBuffer commandBuffer, VkBuffer src,
			const VkExtent3D extent, const VkOffset3D offset = {},
			const std::uint32_t mipLevel = 0, const std::uint32_t layers = 1) const{
			...
			vkCmdCopyImageToBuffer(commandBuffer, handle, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, src, 1, &copy);
		}

...
export namespace Math {
	...
	constexpr T sign(const T f) noexcept requires ext::signed_number<T> {
	...
	template <ext::number T>
	constexpr T sign(const T f) noexcept requires ext::non_negative<T> {
		return f == 0 ? 0 : 1;
	}
	...
	T maxAbs(const T v1, const T v2) noexcept {
		if constexpr (ext::non_negative<T>){
	...
	constexpr T trac(const float value) noexcept {
		...
		if constexpr (ext::signed_number<T>){
			if(value < 0)--val;
		}
